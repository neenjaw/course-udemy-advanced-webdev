...
function countPairs(arr, num){
    // METHOD 1:
    /*
    const walkingSet = new Set();

    let count = 0;

    arr.forEach((el, idx, a) => {
        const complement = num - el;

        if (!walkingSet.has(el) && !walkingSet.has(complement)) {
            walkingSet.add(el);
            walkingSet.add(complement);

            if (arr.includes(complement, idx + 1)) {
                count += 1;
            }
        }
    });
