...
function sumEvenArguments(){
    return Array.prototype.reduce.call(arguments, function(sumOfEven, current){
        if (current % 2 === 0) {
            return sumOfEven + current;
        } else {
            return sumOfEven;
        }
    },0);
...
function invokeMax(fn, num){
    var invocationsLeft = num;

    return function() {
        if (invocationsLeft > 0) {
            invocationsLeft--;

            // return fn.call(this, ...arguments); // Mine using spread operatior
...
function flip(fn, thisArg){
    ...
    return function () {
        ...
        console.log({outerArgs, innerArgs, allArgs});
